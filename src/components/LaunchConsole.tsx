...
import { RigHead, PlateHead } from "./ui/Section";
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1fr)_390px] lg:items-start">
      <div className="space-y-6">
        {/* ---------------------------------------------------------- 01 */}
        <StationCard index="01" title="Identity" note="What the token is called.">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <Field
              ...
              invalid={draft.symbol.length > 0 && symbol.length < limits.symbolMin}
            >
              <input
                className={clsx(
                  "field uppercase",
        ...
        >
          {registryIsEmpty ? (
            <div className="border border-halt/40 bg-halt-soft p-4">
              <Chip tone="halt" lamp blink>
                Registry empty
              </Chip>
              <p className="mt-3 text-[13px] leading-relaxed text-ink-dim">
                No tokenized equity has been verified on {robinhoodChain.name} for
                this registry yet, so there is nothing to pick from a list. You can
                still point a pad at any contract by address — but a pad buys
                whatever it is handed, so an address you have not checked yourself
                is a mistake that spends real fees.
              </p>
              <ul className="mt-4 space-y-2">
                {assetStandard.map((line) => (
                  <li key={line} className="flex gap-3 text-[12px] text-ink-dim">
                    <span aria-hidden className="mono text-halt-ink">
                      ▢
                    </span>
                    <span>{line}</span>
                  </li>
            ...
            <div
              ...
              className="grid grid-cols-1 gap-px bg-rule-2 sm:grid-cols-2"
            >
                ...
                return (
                  <button
                    ...
                    className={clsx(
                      ...
                      selected ? "bg-panel text-panel-ink" : "bg-steel-2 hover:bg-steel-3",
                    )}
                  >
                    <span className="mono block text-[14px]">{asset.symbol}</span>
                    <span
                      className={clsx(
                        "mt-1 block text-[12px]",
                        selected ? "text-panel-dim" : "text-ink-faint",
                      )}
                    >
                      {asset.name}
                    </span>
                    <span
                      className={clsx(
                        ...
                        selected ? "text-panel-faint" : "text-ink-faint",
                      )}
                    >
          ...
          <div className="mt-4">
            ...
            {payout && !selectedFromRegistry && (
              <p className="mt-2 text-[12px] leading-snug text-halt-ink">
                This address is not in the registry. Nothing here has checked that
                it is an ERC-20, that it is transferable, or that it is what you
                think it is.
              </p>
            )}
          </div>
        ...
        >
          <div className="flex flex-wrap items-end gap-4">
            <div>
              ...
              <p className="mono num mt-1 text-[40px] leading-none">
                {bpsToPercent(draft.tradeFeeBps)}
              </p>
            </div>
            <p className="mono mb-1 text-[11px] text-ink-faint">
              {draft.tradeFeeBps} bps · min {bpsToPercent(feeBounds.minTradeFeeBps)}{" "}
              · max {bpsToPercent(feeBounds.maxTradeFeeBps)}
            </p>
          </div>
          <input
            type="range"
            className="detent mt-4"
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {[50, 100, 150, 300, 500]
              .filter(
                (bps) =>
                  bps >= feeBounds.minTradeFeeBps && bps <= feeBounds.maxTradeFeeBps,
              )
              .map((bps) => (
                <button
                  key={bps}
                  ...
                  className={clsx(
                    "mono border px-2.5 py-1.5 text-[10px] tracking-[0.14em] uppercase transition",
                    draft.tradeFeeBps === bps
                      ? "border-ink bg-panel text-panel-ink"
                      : "border-rule-2 text-ink-dim hover:border-halt-ink hover:text-halt-ink",
                  )}
                >
                  {bpsToPercent(bps)}
                </button>
          ...
          <p className="mt-4 text-[13px] leading-relaxed text-ink-dim">
            A high fee funds bigger payouts and makes the token more expensive to
            trade. That trade-off is yours to make and it is fixed at deploy — the
            pad has no function to change it afterwards, so nobody can raise it on
            your holders later, including you.
          </p>
        </StationCard>
        ...
        >
          <div className="flex flex-wrap items-end justify-between gap-4">
            ...
            <p className="mono mb-1 max-w-[280px] text-[11px] leading-snug text-ink-faint">
          ...
          <div className="mt-6">
            ...
            >
              <input
                className={clsx(
                  "field",
                  treasuryInput.length > 0 && treasury === null && "field-invalid",
                )}
      ...
      <aside className="space-y-4 lg:sticky lg:top-24">
        <div className="plate bolted">
          <PlateHead
            ...
            aside={
              <button
                ...
                className="mono text-[10px] tracking-[0.16em] text-ink-dim uppercase transition hover:text-halt-ink"
              >
                {copied ? "Copied" : "Copy"}
              </button>
            }
          ...
          <div className="px-4 py-3">
            ...
            <p className="mono mt-1 text-[12px] text-ink">createPad(</p>
            <dl className="mt-1 space-y-1.5 pl-3">
              {call.args.map((arg) => (
            ...
            <p className="mono mt-1 text-[12px] text-ink">)</p>
            <p className="mono mt-3 border-t border-rule pt-3 text-[10px] tracking-[0.14em] text-ink-faint uppercase">
              Value {call.value.toString()} wei · launching costs gas and nothing
              else
            </p>
          </div>
        </div>
          ...
          <div className="p-4">
            {armed ? (
              <button
                ...
                className="control w-full justify-center border-halt bg-halt py-6 text-[13px] tracking-[0.28em] text-white"
              >
                {isSigning
                  ? "Confirm in wallet…"
                  : isMining
              ...
              <div>
                <div
                  className="hazard flex h-[88px] items-center justify-center border border-black"
                  aria-hidden
                >
                  <span className="mono bg-panel px-4 py-2 text-[12px] tracking-[0.28em] text-panel-ink uppercase">
                    Held
                  </span>
                </div>
            ...
            {writeError && (
              <p className="mono mt-3 text-[11px] leading-snug text-halt-bright">
                {writeError.message}
              </p>
            )}
            {hash && (
              <p className="mono mt-3 text-[11px] break-all text-panel-dim">
        ...
        <div className="flex items-center justify-between gap-3 border border-rule-2 bg-steel-2 px-4 py-3">
          <p className="text-[11px] leading-snug text-ink-faint">
            This draft is saved in this browser only. There is no backend on this
            site.
          </p>
          <button
            type="button"
            onClick={reset}
            className="mono shrink-0 text-[10px] tracking-[0.16em] text-ink-dim uppercase transition hover:text-halt-ink"
          >
            Clear
          </button>
        </div>
...
function StationCard({
  ...
  return (
    <section className="plate bolted">
      <PlateHead title={`Station ${index}`} aside={<span>{title}</span>} />
      <div className="p-5">
        <p className="mb-5 text-[13px] leading-relaxed text-ink-dim">{note}</p>
        {children}
      </div>
    </section>
  );
  ...
  return (
    <label className="block">
      ...
      {hint && (
        <span
          className={clsx(
            "mono mt-1.5 block text-[10px] leading-snug",
            invalid ? "text-halt-ink" : "text-ink-faint",
          )}
        >
          {hint}
        </span>
