...
import { RigHead } from "./ui/Section";
...
export function PadRegistry() {
  ...
  const pads = (addresses ?? [])
    .map((entry) => (entry.status === "success" ? (entry.result as `0x${string}`) : null))
    ...
    return (
      <div className="rig bolted">
        ...
        <div className="p-8 text-center">
          <p className="mono num text-[56px] leading-none text-panel-ink">0</p>
          <p className="label label-on-panel mt-3">Pads launched</p>
          <p className="mx-auto mt-5 max-w-md text-[13px] leading-relaxed text-panel-dim">
            This table reads the factory contract directly. No factory is
            deployed, so there is nothing to read and nothing is shown — the zero
            above is the absence of a contract, not a slow start.
          </p>
          <p className="mono mx-auto mt-4 max-w-md text-[11px] leading-relaxed text-panel-faint">
            When one exists, each row will carry the pad address, its payout
            asset, its fee, its split, and the total it has distributed, all read
            from chain.
          </p>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[640px] border-collapse">
                ...
                {["Pad", "Payout", "Fee", "To holders", "Distributed"].map(
                  (heading) => (
                    <th
                      key={heading}
                      scope="col"
                      className="label label-on-panel px-4 py-3 text-left"
                    >
                      {heading}
                    </th>
                  ),
            ...
            <tbody>
              {pads.map((pad, index) => {
                const base = index * 4;
                const asset = details?.[base]?.result as `0x${string}` | undefined;
                const feeBps = details?.[base + 1]?.result as number | undefined;
                const creatorBps = details?.[base + 2]?.result as number | undefined;
                const distributed = details?.[base + 3]?.result as bigint | undefined;
                const holderBps =
                ...
                return (
                  <tr key={pad} className="border-b border-panel-rule last:border-0">
                    <td className="px-4 py-3">
                      <a
                        href={explorer.address(pad)}
                        target="_blank"
                        rel="noreferrer noopener"
                        className="mono text-[12px] text-panel-ink underline"
                      >
                        {shortAddress(pad)}
                      </a>
                    </td>
                    <td className="mono px-4 py-3 text-[12px] text-panel-dim">
                      {known ? known.symbol : asset ? shortAddress(asset) : "—"}
                    </td>
                    <td className="mono num px-4 py-3 text-[12px] text-panel-dim">
                      {feeBps === undefined ? "—" : bpsToPercent(feeBps)}
                    </td>
                    <td className="mono num px-4 py-3 text-[12px] text-live-bright">
