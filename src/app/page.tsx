...
export default function Home() {
  return (
    <>
      ...
      <Section id="mechanism">
        <div data-reveal>
          <StationHead
            ...
            aside={<Chip tone="muted">Schematic</Chip>}
          />
          <div className="plate bolted overflow-x-auto p-4 sm:p-6">
            <RouteDiagram
          ...
          <p className="mono mt-3 text-[10px] tracking-[0.14em] text-ink-faint uppercase">
          ...
          <div className="mt-10 grid grid-cols-1 gap-8 sm:grid-cols-3">
            {[
              ...
              <div key={item.head}>
                <span className="mono text-[11px] text-halt-ink">
                  {String(index + 1).padStart(2, "0")}
                </span>
                <h3 className="stencil mt-2 text-[17px]">{item.head}</h3>
      ...
      {/* ------------------------------------------------------------ 02 */}
      <Section id="split" className="border-t border-rule-2">
        <div data-reveal>
          <StationHead
            index="02"
            title="Where the fee goes"
            aside={<Chip tone="halt">Proposed terms</Chip>}
          />
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-start">
            <div>
              ...
              <p className="mt-4 max-w-2xl text-[14px] leading-relaxed text-ink-dim">
                Launching costs gas and nothing else. There is no listing fee, no
                token sale, and the protocol never takes a share of your supply —
                only of the fees your pad produces. If your pad never trades,
                this launchpad earns nothing from it, which is the whole reason
                the door is free.
              </p>
            </div>
            ...
            <div className="plate bolted p-5">
              ...
              <ul className="mt-4 space-y-3">
                {[
                  ...
                  <li key={item} className="flex items-center gap-3">
                    ...
                    <span className="mono text-[12px] tracking-[0.08em] uppercase">
                      {item}
                    </span>
                  </li>
                ))}
              ...
              <p className="mt-5 border-t border-rule pt-4 text-[12px] leading-relaxed text-ink-faint">
                None of these has a setter. A pad is a finished object the moment
                it is deployed — that is the guarantee being sold here, and it is
                also the reason to get the numbers right before you press the
                control.
              </p>
            </div>
          </div>
      ...
      {/* ------------------------------------------------------------ 03 */}
      <Section id="one-click" className="border-t border-rule-2">
        <div data-reveal>
          <StationHead
            index="03"
            title="What one click actually does"
            aside={<Chip tone="muted">One transaction</Chip>}
          />
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-2">
            <ol className="rail space-y-6 pl-6">
              {[
                {
                ...
                <li key={step.head} className="relative">
                  <span className="mono absolute -left-6 top-0.5 text-[11px] text-halt-ink">
                    {String(index + 1).padStart(2, "0")}
                  </span>
                  <h3 className="stencil text-[16px]">{step.head}</h3>
            ...
            <div className="rig bolted self-start">
              <div className="rig-head">
                <span>What it does not do</span>
                <Chip tone="panel">Read this</Chip>
              </div>
              <ul className="divide-y divide-panel-rule">
                {[
                  ...
                  <li key={item.head} className="px-5 py-4">
                    <h3 className="mono text-[12px] tracking-[0.08em] text-halt-bright uppercase">
                      {item.head}
                    </h3>
                    <p className="mt-1.5 text-[13px] leading-relaxed text-panel-dim">
                      {item.body}
                    </p>
                  </li>
                ))}
      ...
      {/* ------------------------------------------------------------ 04 */}
      <Section id="chain" className="border-t border-rule-2">
        <div data-reveal>
          <StationHead
            index="04"
            title="The chain is live. This is not."
            aside={<Chip tone="muted">Read by your browser</Chip>}
          />
          <ChainReadout />
          <p className="mt-4 max-w-2xl text-[14px] leading-relaxed text-ink-dim">
      ...
      {/* ------------------------------------------------------------ 05 */}
      <Section id="registry" className="border-t border-rule-2">
        <div data-reveal>
          <StationHead
            index="05"
            title="Every pad ever launched"
            aside={<Chip tone="muted">Read from the factory</Chip>}
          />
          <PadRegistry />
          <div className="mt-6">
            <Link
      ...
      {/* ------------------------------------------------------------ 06 */}
      <Section id="questions" className="border-t border-rule-2">
        <div data-reveal>
      ...
      <section className="border-t border-rule-2 bg-panel">
        <div className="mx-auto w-full max-w-[1180px] px-4 py-16 sm:px-6 sm:py-20">
          <h2 className="stencil max-w-3xl text-[clamp(1.7rem,4vw,2.6rem)] leading-[1.02] text-panel-ink text-balance">
            The console runs today. The chain will refuse it today.
          </h2>
          <p className="mt-5 max-w-2xl text-[15px] leading-relaxed text-panel-dim">
            Fill in a pad, watch the interlocks close one by one, read the exact
            call {siteConfig.wordmark} would send. The last interlock is a
            factory contract that does not exist yet — and until it does, that is
            the only thing standing between the draft and a deployment.
          </p>
          <div className="mt-8">
            <Link
              href="/launch"
